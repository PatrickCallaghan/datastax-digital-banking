...
public class RunRequests {
	...
	public RunRequests() {
		...
		for (int i = 0; i < noOfRequests; i++) {
			String search = BankGenerator.tagList.get(new Double(Math.random() * BankGenerator.tagList.size()).intValue());
			Set<String> tags = new HashSet<String>();
			tags.add(search);

			Transaction t = new Transaction();
			t.setAccountNo(new Double(Math.random() * noOfCreditCards).intValue() + "");
